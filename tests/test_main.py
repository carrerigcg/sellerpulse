...
from src.storage import connect, get_orders_in_range, get_claims_in_range
...
def test_ingest_window_persists_orders_items_claims_and_logs_run(tmp_path, monkeypatch):
    ...
    responses.add(
        ...
        json={"paging": {"total": 1, "offset": 0, "limit": 50},
              "results": [{
                  "id": 1001,
                  ...
                  "buyer": {"id": 8888},
                  "order_items": [
                      {"item": {"id": "MLB1"}, "quantity": 1, "unit_price": 250.0}
                  ],
              }]},
        status=200,
    )
    ...
    responses.add(
        responses.GET, f"{BASE_URL}/items/MLB1",
        json={"id": "MLB1", "title": "Lanterna CG 150", "category_id": "MLB-cat-A"},
        status=200,
    )
    responses.add(
        responses.GET, f"{BASE_URL}/categories/MLB-cat-A",
        json={"id": "MLB-cat-A", "name": "Iluminação"}, status=200,
    )
    responses.add(
        responses.GET, f"{BASE_URL}/post-purchase/v1/claims/search",
        json={"data": [], "paging": {"total": 0}}, status=200,
    )
    responses.add(
        responses.GET, f"{BASE_URL}/users/999",
        json={"id": 999, "seller_reputation": {
            "level_id": "5_green", "metrics": {"claims": {"value": 0}}}},
        status=200,
    )

    # 3) Executar
    result = ingest_window(
        tokens_path=tokens_path, db_path=db_path,
